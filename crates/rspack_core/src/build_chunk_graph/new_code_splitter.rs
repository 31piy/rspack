...
use crate::{
  assign_depths, is_runtime_equal, merge_runtime, AsyncDependenciesBlockIdentifier, ChunkGroup,
    ...
    match self {
      CreateChunkRoot::Entry(entry, data, runtime) => {
        ...
        let chunk_loading = matches!(
          compilation.options.output.chunk_loading,
          ChunkLoading::Enable(_)
        );
        ...
        for m in deps
          ...
          .filter_map(|dep_id| module_graph.module_identifier_by_dependency_id(dep_id))
        {
          splitter.fill_chunk_modules(*m, chunk_loading, runtime.as_ref(), &module_graph, &mut ctx);
        }
      ...
      CreateChunkRoot::Block(origin, block_id, runtime) => {
        ...
        let chunk_loading = block
          .get_group_options()
          .and_then(|option| option.entry_options())
          .and_then(|entry_option| entry_option.chunk_loading.clone())
          .map(|chunk_loading| matches!(chunk_loading, ChunkLoading::Enable(_)))
          .unwrap_or(true);
...
impl CodeSplitter {
  ...
  fn fill_chunk_modules(
    ...
    blocks.retain(|block| {
      if !chunk_loading && let Some(block) = module_graph.block_by_id(block) {
    ...
    for m in outgoing_modules.iter() {
      self.fill_chunk_modules(*m, chunk_loading, runtime, module_graph, ctx);
    }
